...
namespace myutils
{
	...
	// append data
	//
	void MD5Helper::appendData(void const* inputBuffer, size_t inputLength)
	{
		MD5_Update(&mContext, static_cast<unsigned char const*>(inputBuffer), inputLength);
	}

	//
	// get hash result
	//
	void MD5Helper::getHashResult(void* outputBuffer)
	{
		MD5_Final(static_cast<uint8_t*>(outputBuffer), &mContext);
	}

	//
	// hash data
	//
	void MD5Helper::hashData(void const* inputBuffer, size_t inputLength, void* outputBuffer)
	{
		reinitialize();
		appendData(inputBuffer, inputLength);
		getHashResult(outputBuffer);
	}
}
